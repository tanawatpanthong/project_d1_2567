<template>
    ...
    <div class="text-center pa-4">
        <v-dialog v-model="dialog_error" width="auto">
            <v-card max-width="400" prepend-icon="mdi-update" text="กรุณาตรวจสอบ Username และ Password" title="สถานะ">
                <template v-slot:actions>
                    <v-btn class="ms-auto" text="Ok" @click="dialog_error = false"></v-btn>
                </template>
            </v-card>
        </v-dialog>
    </div>
    <div>
        ...
        <v-card class="mx-auto pa-12 pb-8" elevation="8" max-width="448" rounded="lg">
            ...
            <v-text-field :append-inner-icon="visible ? 'mdi-eye-off' : 'mdi-eye'" :type="visible ? 'text' : 'password'"
                ...
                variant="outlined" v-model="password" @click:append-inner="visible = !visible"></v-text-field>
...
export default {
    ...
    methods: {
        async dologin() {
            ...
            const response = await axios.post('http://localhost:7000/login', froms)
